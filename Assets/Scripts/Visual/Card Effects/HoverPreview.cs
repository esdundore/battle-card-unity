...
public class HoverPreview: MonoBehaviour
{
    ...
    public GameObject CardFront; // do not preview if the card is not facing you
    ...
    {
        OverCollider = true;
        if (PreviewsAllowed && /*ThisPreviewEnabled &&*/ CardFront.activeSelf)
            PreviewThisObject();
    }
    ...
    void PreviewThisObject()
    {
        ...
        StopAllPreviews();
        // 1) save this HoverPreview as curent
        currentlyViewing = this;
        // 2) enable Preview game object
        previewGameObject.SetActive(true);
        // 3) disable if we have what to disable
        if (TurnThisOffWhenPreviewing!=null)
            TurnThisOffWhenPreviewing.SetActive(false); 
        // 4) tween to target position
        previewGameObject.transform.localPosition = Vector3.zero;
        previewGameObject.transform.localScale = Vector3.one;

        previewGameObject.transform.DOMove(TargetPosition, 1f).SetEase(Ease.OutQuint);
